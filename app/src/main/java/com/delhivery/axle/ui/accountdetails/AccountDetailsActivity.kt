...
class AccountDetailsActivity :BaseActivity<ActivityAccountDetailsBinding, AccountDetailsViewModel>() {
    ...
    fun checkEnable() {
        binding.btnCreateAccount.isEnabled = (viewModel.firstName.value?.trim().isNotNullOrEmpty() && viewModel.firstName.value?.trim().isNotNullOrEmpty() && viewModel.commConsent.value == true)
    }
