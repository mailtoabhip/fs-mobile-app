...
class DelhiveryAutofitHelper private constructor(
  ...
  /**
   * When an object of a type is attached to an `DelhiveryAutofitHelper`, its methods will be called
  ...
  companion object {
    ...
    /**
     * Creates a new instance of `DelhiveryAutofitHelper` that wraps a [TextView] and enables
     * automatically sizing the text to fit.
     */
    @JvmOverloads fun create(
      view: TextView,
      ...
      if (attrs != null) {
        ...
        var minTextSize = helper.getMinTextSize()
            .toInt()
    ...
    private fun autofit(
      ...
      if (maxLines == 1 && paint.measureText(text, 0, text.length) > targetWidth || getLineCount(
              text, paint, size, targetWidth.toFloat(), displayMetrics
          ) > maxLines
      ) {
