...
import com.dfd.delfin.api.service.FasTAGKycService
import com.dfd.delfin.api.service.FastagService
import com.dfd.delfin.injection.qualifier.IoDispatcher
import com.dfd.delfin.ui.fastag.tagAssignment.pendingActions.PendingActionsResponse
...
class FastagRepository @Inject constructor(
     ...
     */
    suspend fun searchProductBarcode(journeyId: String, barcode: String): Resource<com.dfd.delfin.api.response.ProductBarcodeResponse> =
        safeApiCall {
            kycService.searchProductBarcode(
                com.dfd.delfin.api.request.ProductBarcodeRequest(journeyId, barcode)
            ).toResource()
        }
    ...
    suspend fun generateOtp(journeyId: String, barcode: String, tagId: String): Resource<Any> =
        safeApiCall {
            kycService.generateOtp(
                com.dfd.delfin.api.request.GenerateOtpRequest(journeyId, barcode, tagId)
            ).toResource()
        }
    ...
    suspend fun issueTag(
        ...
        barcode: String,
        otp: String
    ): Resource<com.dfd.delfin.api.response.IssueTagResponse> =
        safeApiCall {
    ...
    suspend fun validateFastagImage(journeyId: String, orderId: String, orderItemId: String): Resource<FastagImageValidateResponse> =
        withContext(ioDispatcher) {
            safeApiCall {
                val request = com.dfd.delfin.api.request.FastagImageValidateRequest(
                    journeyId = journeyId,
                    orderId = orderId,
